genuine__attributes = class ({})
LinkLuaModifier("genuine__modifier_effect", "heroes/genuine/genuine__modifier_effect", LUA_MODIFIER_MOTION_NONE)
require("talent_tree")

--TALENT FUNCTIONS
	genuine__attributes.talents = {
		[1] = {
	...
	genuine__attributes.skills = {
		[1] = "genuine_1__shooting",
		[2] = "genuine_2__fallen",
		[3] = "genuine_3__morning",
		[4] = "genuine_u__star"
	}

	function genuine__attributes:UpgradeRank(skill, id, level)
		...
		local ability = nil
		if skill == 1 then ability = caster:FindAbilityByName("genuine_1__shooting") end
		if skill == 2 then ability = caster:FindAbilityByName("genuine_2__fallen") end
		if skill == 3 then ability = caster:FindAbilityByName("genuine_3__morning") end
		if skill == 4 then ability = caster:FindAbilityByName("genuine_u__star") end
	...
	function genuine__attributes:CheckNewAbility(skill, id, level)
		local caster = self:GetCaster()

		if skill == 5 and id == 1 then caster:FindAbilityByName("genuine_x1__nightfall"):SetLevel(level) end
		if skill == 5 and id == 2 then caster:FindAbilityByName("genuine_x2__under"):SetLevel(level) end
	end

--SHOW ATTRIBUTES FUNCTIONS
	function genuine__attributes:GetIntrinsicModifierName()
		return "genuine__modifier_effect"
	end

	function genuine__attributes:GetAbilityTextureName()
	...
	function genuine__attributes:OnToggle()
		local caster = self:GetCaster()
		local ATTs = {
...
--LEVELUP ATTRIBUTES FUNCTIONS
	function genuine__attributes:Spawn()
		local caster = self:GetCaster()
	...
	function genuine__attributes:OnHeroLevelUp()
		local caster = self:GetCaster()
	...
	function genuine__attributes:OnUpgrade()
		local caster = self:GetCaster()
	...
	function genuine__attributes:SetBasicFraction(secondary_att, fraction)
		local caster = self:GetCaster()
