lawbreaker_x_modifier_example = class({})

function lawbreaker_x_modifier_example:IsHidden() return false end
function lawbreaker_x_modifier_example:IsPurgable() return false end

-- CONSTRUCTORS -----------------------------------------------------------

function lawbreaker_x_modifier_example:OnCreated(kv)
    self.caster = self:GetCaster()
    self.parent = self:GetParent()
    self.ability = self:GetAbility()
	
	self.caster:AddNewModifier(self.caster, self.ability, "lawbreaker_x_modifier_spike_caster", {
...
function lawbreaker_x_modifier_example:OnRefresh(kv)
	self.caster:AddNewModifier(self.caster, self.ability, "lawbreaker_x_modifier_spike_caster", {
		duration = self:GetDuration()
	})
end

function lawbreaker_x_modifier_example:OnRemoved()
	local mod_caster = self.caster:FindModifierByNameAndCaster("lawbreaker_x_modifier_spike_caster", self.caster)
	if mod_caster then mod_caster:DecrementStackCount() end
end

-- API FUNCTIONS -----------------------------------------------------------

function lawbreaker_x_modifier_example:DeclareFunctions()
	local funcs = {
...
function lawbreaker_x_modifier_example:GetAbsorbSpell(keys)
...
function lawbreaker_x_modifier_example:GetModifierIncomingDamage_Percentage(keys)
...
function lawbreaker_x_modifier_example:GetEffectName()
	return "particles/items3_fx/star_emblem_friend_shield.vpcf"
end

function lawbreaker_x_modifier_example:GetEffectAttachType()
	return PATTACH_OVERHEAD_FOLLOW
end

function lawbreaker_x_modifier_example:PlayEfxStart()
	if IsServer() then self.parent:EmitSound("Item.StarEmblem.Friendly") end
end

function lawbreaker_x_modifier_example:PlayEfxHit()
