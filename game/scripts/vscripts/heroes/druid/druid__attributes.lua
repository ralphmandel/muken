druid__attributes = class ({})
LinkLuaModifier("druid__modifier_effect", "heroes/druid/druid__modifier_effect", LUA_MODIFIER_MOTION_NONE)
require("talent_tree")

--TALENT FUNCTIONS
	druid__attributes.talents = {
		[1] = {[0] = false, [1] = false, [2] = false, [3] = false, [4] = false, [5] = false, [6] = false, [7] = false, [8] = "druid_1__conversion"},
		[2] = {[0] = false, [1] = false, [2] = false, [3] = false, [4] = false, [5] = false, [6] = false, [7] = false, [8] = "druid_2__root"},
		[3] = {[0] = false, [1] = false, [2] = false, [3] = false, [4] = false, [5] = false, [6] = false, [7] = false, [8] = "druid_3__totem"},
		[4] = {[0] = false, [1] = false, [2] = false, [3] = false, [4] = false, [5] = false, [6] = false, [7] = false, [8] = "druid_u__seed"}
	}

	function druid__attributes:UpgradeRank(skill, id, level)
		...
		local ability = nil
		if skill == 1 then ability = caster:FindAbilityByName("druid_1__conversion") end
		if skill == 2 then ability = caster:FindAbilityByName("druid_2__root") end
		if skill == 3 then ability = caster:FindAbilityByName("druid_3__totem") end
		if skill == 4 then ability = caster:FindAbilityByName("druid_u__seed") end
	...
	function druid__attributes:CheckNewAbility(skill, id, level)
		local caster = self:GetCaster()

		if skill == 5 and id == 1 then caster:FindAbilityByName("druid_x1__tree"):SetLevel(level) end
		if skill == 5 and id == 2 then caster:FindAbilityByName("druid_x2__form"):SetLevel(level) end
	end

--SHOW ATTRIBUTES FUNCTIONS
	function druid__attributes:GetIntrinsicModifierName()
		return "druid__modifier_effect"
	end

	function druid__attributes:GetAbilityTextureName()
	...
	function druid__attributes:OnToggle()
		local caster = self:GetCaster()
		local ATTs = {
...
--LEVELUP ATTRIBUTES FUNCTIONS
	function druid__attributes:Spawn()
		local caster = self:GetCaster()
	...
	function druid__attributes:OnHeroLevelUp()
		local caster = self:GetCaster()
	...
	function druid__attributes:OnUpgrade()
		local caster = self:GetCaster()
	...
	function druid__attributes:SetBasicFraction(secondary_att, fraction)
		local caster = self:GetCaster()
