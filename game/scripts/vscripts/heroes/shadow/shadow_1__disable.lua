...
-- INIT
    ...
    function shadow_1__disable:CalcStatus(duration, caster, target)
        local time = duration
    ...
    function shadow_1__disable:AddBonus(string, target, const, percent, time)
        local att = target:FindAbilityByName(string)
        if att then att:BonusPts(self:GetCaster(), self, const, percent, time) end
    end

    function shadow_1__disable:RemoveBonus(string, target)
    ...
    function shadow_1__disable:GetRank(upgrade)
    ...
    function shadow_1__disable:OnUpgrade()
        local caster = self:GetCaster()
        if caster:IsIllusion() then return end
        if caster:GetUnitName() ~= "npc_dota_hero_phantom_assassin" then return end
