...
-- INIT
    ...
    function shadow_1__strike:AddBonus(string, target, const, percent, time)
        local att = target:FindAbilityByName(string)
        if att then att:BonusPts(self:GetCaster(), self, const, percent, time) end
    end

    function shadow_1__strike:RemoveBonus(string, target)
    ...
    function shadow_1__strike:GetRank(upgrade)
    ...
    function shadow_1__strike:OnUpgrade()
        local caster = self:GetCaster()
        if caster:IsIllusion() then return end
        if caster:GetUnitName() ~= "npc_dota_hero_spectre" then return end
    ...
    function shadow_1__strike:Spawn()
        self:SetCurrentAbilityCharges(0)
    end

-- SPELL START

    function shadow_1__strike:GetIntrinsicModifierName()
        return "shadow_1_modifier_passive"
    end
