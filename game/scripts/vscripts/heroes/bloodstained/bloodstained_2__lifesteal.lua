...
-- INIT
    ...
    function bloodstained_2__lifesteal:CalcStatus(duration, caster, target)
    ...
    function bloodstained_2__lifesteal:AddBonus(string, target, const, percent, time)
        local base_stats = target:FindAbilityByName("base_stats")
        if base_stats then base_stats:AddBonusStat(self:GetCaster(), self, const, percent, time, string) end
    end

    function bloodstained_2__lifesteal:RemoveBonus(string, target)
    ...
    function bloodstained_2__lifesteal:GetRank(upgrade)
    ...
    function bloodstained_2__lifesteal:OnUpgrade()
        local caster = self:GetCaster()
        if caster:IsIllusion() then return end
        if caster:GetUnitName() ~= "npc_dota_hero_shadow_demon" then return end
