succubus__attributes = class ({})
LinkLuaModifier("succubus__modifier_effect", "heroes/succubus/succubus__modifier_effect", LUA_MODIFIER_MOTION_NONE)
require("talent_tree")

--TALENT FUNCTIONS
	succubus__attributes.talents = {
		[1] = {
	...
	function succubus__attributes:UpgradeRank(skill, id, level)
		...
		local ability = nil
		if skill == 1 then ability = caster:FindAbilityByName("succubus_1__dash") end
		if skill == 2 then ability = caster:FindAbilityByName("succubus_2__heart") end
		if skill == 3 then ability = caster:FindAbilityByName("succubus_3__clone") end
		if skill == 4 then ability = caster:FindAbilityByName("succubus_u__bat") end
	...
	function succubus__attributes:CheckNewAbility(skill, id, level)
		local caster = self:GetCaster()

		if skill == 5 and id == 1 then caster:FindAbilityByName("succubus_x1__extra1"):SetLevel(level) end
		if skill == 5 and id == 2 then caster:FindAbilityByName("succubus_x2__extra2"):SetLevel(level) end
	end

--SHOW ATTRIBUTES FUNCTIONS
	function succubus__attributes:GetIntrinsicModifierName()
		return "succubus__modifier_effect"
	end

	function succubus__attributes:GetAbilityTextureName()
	...
	function succubus__attributes:OnToggle()
		local caster = self:GetCaster()
		local ATTs = {
...
--LEVELUP ATTRIBUTES FUNCTIONS
	function succubus__attributes:Spawn()
		local caster = self:GetCaster()
	...
	function succubus__attributes:OnHeroLevelUp()
		local caster = self:GetCaster()
	...
	function succubus__attributes:OnUpgrade()
		local caster = self:GetCaster()
	...
	function succubus__attributes:SetBasicFraction(secondary_att, fraction)
		local caster = self:GetCaster()
