athena_2__thunder = class({})
LinkLuaModifier("athena_2_modifier_thunder", "heroes/athena/athena_2_modifier_thunder", LUA_MODIFIER_MOTION_NONE)

-- INIT

    function athena_2__thunder:CalcStatus(duration, caster, target)
        local time = duration
    ...
    function athena_2__thunder:AddBonus(string, target, const, percent, time)
        local att = target:FindAbilityByName(string)
        if att then att:BonusPts(self:GetCaster(), self, const, percent, time) end
    end

    function athena_2__thunder:RemoveBonus(string, target)
    ...
    function athena_2__thunder:GetRank(upgrade)
    ...
    function athena_2__thunder:OnUpgrade()
        local caster = self:GetCaster()
        if caster:IsIllusion() then return end
        if caster:GetUnitName() ~= "npc_dota_hero_lina" then return end
    ...
    function athena_2__thunder:Spawn()
        self:SetCurrentAbilityCharges(0)
    end

-- SPELL START

    function athena_2__thunder:OnSpellStart()
        local caster = self:GetCaster()
    end
