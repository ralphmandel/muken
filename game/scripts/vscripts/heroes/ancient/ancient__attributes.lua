ancient__attributes = class ({})
LinkLuaModifier("ancient__modifier_effect", "heroes/ancient/ancient__modifier_effect", LUA_MODIFIER_MOTION_NONE)
require("talent_tree")

--TALENT FUNCTIONS
	ancient__attributes.talents = {
		[1] = {
	...
	ancient__attributes.skills = {
		[1] = "ancient_1__berserk",
		[2] = "ancient_2__leap",
		[3] = "ancient_3__aura",
		[4] = "ancient_u__final"
	}

	function ancient__attributes:UpgradeRank(skill, id, level)
		...
		local ability = nil
		if skill == 1 then ability = caster:FindAbilityByName("ancient_1__berserk") end
		if skill == 2 then ability = caster:FindAbilityByName("ancient_2__leap") end
		if skill == 3 then ability = caster:FindAbilityByName("ancient_3__aura") end
		if skill == 4 then ability = caster:FindAbilityByName("ancient_u__final") end
	...
	function ancient__attributes:CheckNewAbility(skill, id, level)
		local caster = self:GetCaster()

		if skill == 5 and id == 1 then caster:FindAbilityByName("ancient_x1__extra1"):SetLevel(level) end
		if skill == 5 and id == 2 then caster:FindAbilityByName("ancient_x2__extra2"):SetLevel(level) end
	end

--SHOW ATTRIBUTES FUNCTIONS
	function ancient__attributes:GetIntrinsicModifierName()
		return "ancient__modifier_effect"
	end

	function ancient__attributes:GetAbilityTextureName()
	...
	function ancient__attributes:OnToggle()
		local caster = self:GetCaster()
		local ATTs = {
...
--LEVELUP ATTRIBUTES FUNCTIONS
	function ancient__attributes:Spawn()
		local caster = self:GetCaster()
	...
	function ancient__attributes:OnHeroLevelUp()
		local caster = self:GetCaster()
	...
	function ancient__attributes:OnUpgrade()
		local caster = self:GetCaster()
	...
	function ancient__attributes:SetBasicFraction(secondary_att, fraction)
		local caster = self:GetCaster()
