...
function ancient_1_modifier_berserk:GetModifierTotalDamageOutgoing_Percentage(keys)
	...
	if str_mod then
		if str_mod:HasCritical() then
			local stun_duration = self.ability:CalcStatus(keys.original_damage * self.stun_percent, nil, keys.target)
			keys.target:AddNewModifier(self.caster, self.ability, "_modifier_stun", {duration = stun_duration})
	
			if IsServer() then keys.target:EmitSound("DOTA_Item.SkullBasher") end
		end
	end
end