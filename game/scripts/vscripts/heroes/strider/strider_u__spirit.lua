strider_u__spirit = class({})
LinkLuaModifier( "strider_u_modifier_spirit", "heroes/strider/strider_u_modifier_spirit", LUA_MODIFIER_MOTION_NONE )

function strider_u__spirit:CalcStatus(duration, caster, target)
    local time = duration
...
function strider_u__spirit:RemoveBonus(string, target)
...
function strider_u__spirit:Spawn()
	self:SetCurrentAbilityCharges(0)
end

function strider_u__spirit:OnUpgrade()
...
function strider_u__spirit:GetRank(upgrade)
    local caster = self:GetCaster()
    if caster:IsIllusion() then return end
    local att = caster:FindAbilityByName("strider__attributes")
...
function strider_u__spirit:OnSpellStart()
	local caster = self:GetCaster()
end