strider__attributes = class ({})
LinkLuaModifier("strider__modifier_effect", "heroes/strider/strider__modifier_effect", LUA_MODIFIER_MOTION_NONE)
require("talent_tree")

--TALENT FUNCTIONS
	strider__attributes.talents = {
	...
	function strider__attributes:UpgradeRank(skill, id, level)
		...
		local ability = nil
		if skill == 1 then ability = caster:FindAbilityByName("strider_1__push") end
		if skill == 2 then ability = caster:FindAbilityByName("strider_2__dash") end
		if skill == 3 then ability = caster:FindAbilityByName("strider_3__wall") end
		if skill == 4 then ability = caster:FindAbilityByName("strider_u__spirit") end
	...
	function strider__attributes:CheckNewAbility(skill, id, level)
		local caster = self:GetCaster()

		if skill == 5 and id == 1 then caster:FindAbilityByName("strider_x1__seal"):SetLevel(level) end
		if skill == 5 and id == 2 then caster:FindAbilityByName("strider_x2__swap"):SetLevel(level) end
	end

--SHOW ATTRIBUTES FUNCTIONS
	function strider__attributes:GetIntrinsicModifierName()
		return "strider__modifier_effect"
	end

	function strider__attributes:GetAbilityTextureName()
	...
	function strider__attributes:OnToggle()
		local caster = self:GetCaster()
		local ATTs = {
...
--LEVELUP ATTRIBUTES FUNCTIONS
	function strider__attributes:Spawn()
		local caster = self:GetCaster()
	...
	function strider__attributes:OnHeroLevelUp()
		local caster = self:GetCaster()
	...
	function strider__attributes:OnUpgrade()
		local caster = self:GetCaster()
	...
	function strider__attributes:SetBasicFraction(secondary_att, fraction)
		local caster = self:GetCaster()
