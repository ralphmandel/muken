strider_3__wall = class({})
LinkLuaModifier("strider_3_modifier_wall", "heroes/strider/strider_3_modifier_wall", LUA_MODIFIER_MOTION_NONE )

function strider_3__wall:CalcStatus(duration, caster, target)
    local time = duration
...
function strider_3__wall:RemoveBonus(string, target)
...
function strider_3__wall:Spawn()
	self:SetCurrentAbilityCharges(0)
end

function strider_3__wall:OnUpgrade()
...
function strider_3__wall:GetRank(upgrade)
    local caster = self:GetCaster()
    if caster:IsIllusion() then return end
    local att = caster:FindAbilityByName("strider__attributes")
...
function strider_3__wall:OnSpellStart()
	local caster = self:GetCaster()
end