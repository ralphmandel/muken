doctor_1__mask = class({})
LinkLuaModifier("doctor_1_modifier_mask", "heroes/doctor/doctor_1_modifier_mask", LUA_MODIFIER_MOTION_NONE)

-- INIT

    function doctor_1__mask:CalcStatus(duration, caster, target)
        local time = duration
    ...
    function doctor_1__mask:AddBonus(string, target, const, percent, time)
        local att = target:FindAbilityByName(string)
        if att then att:BonusPts(self:GetCaster(), self, const, percent, time) end
    end

    function doctor_1__mask:RemoveBonus(string, target)
    ...
    function doctor_1__mask:GetRank(upgrade)
    ...
    function doctor_1__mask:OnUpgrade()
        local caster = self:GetCaster()
        if caster:IsIllusion() then return end
        if caster:GetUnitName() ~= "npc_dota_hero_rubick" then return end
    ...
    function doctor_1__mask:Spawn()
        self:SetCurrentAbilityCharges(0)
    end

-- SPELL START

    function doctor_1__mask:OnSpellStart()
        local caster = self:GetCaster()
    end
