_1_INT = class ({})
LinkLuaModifier( "_1_INT_modifier", "modifiers/_1_INT_modifier", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "_1_INT_modifier_stack", "modifiers/_1_INT_modifier_stack", LUA_MODIFIER_MOTION_NONE )

function _1_INT:GetIntrinsicModifierName()
	return "_1_INT_modifier"
end

function _1_INT:OnUpgrade()
	self:CalcBase()
end

function _1_INT:SetBasePts(base)
	self.base = base
	self:CalcBase()
end

function _1_INT:CalcBase()
	local caster = self:GetCaster()

	local mod = caster:FindModifierByName("_1_INT_modifier")
	if mod then mod:Base_INT(self.base) end
	self:CalculateAttributes(0, 0)
end

function _1_INT:AddFraction(value)
...
function _1_INT:OnOwnerSpawned()
	if self:GetCaster():IsIllusion() then return end
	local mods = self:GetCaster():FindAllModifiersByName("_1_INT_modifier_stack")
...
function _1_INT:BonusPermanent(value)
	self.permanent = self.permanent + value
	self:CalculateAttributes(0, 0)
end

function _1_INT:BonusPts(caster, inflictor, stacks, percent, duration)
	local target = self:GetCaster()

	if IsServer() then
		target:AddNewModifier(caster, inflictor, "_1_INT_modifier_stack", {
			duration = duration, stacks = stacks, percent = percent
		})
	end
end

function _1_INT:CalculateAttributes(stacks, percent)
	...
	if IsServer() then
		local mod = self:GetCaster():FindModifierByName("_1_INT_modifier")
...
function _1_INT:SetBounds(min, max)
	self.min = min
	self.max = max
	self:CalculateAttributes(0, 0)
end