_2_DEF = class ({})
LinkLuaModifier( "_2_DEF_modifier", "modifiers/_2_DEF_modifier", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "_2_DEF_modifier_stack", "modifiers/_2_DEF_modifier_stack", LUA_MODIFIER_MOTION_NONE )

function _2_DEF:GetIntrinsicModifierName()
	return "_2_DEF_modifier"
end

function _2_DEF:GetBehavior()
...
function _2_DEF:OnOwnerSpawned()
	if self:GetCaster():IsIllusion() then return end
	local mods = self:GetCaster():FindAllModifiersByName("_2_DEF_modifier_stack")
...
function _2_DEF:OnHeroLevelUp()
	local caster = self:GetCaster()
	
	self:CheckLevelUp(false)
end

function _2_DEF:CheckLevelUp(trained)
	local caster = self:GetCaster()
...
function _2_DEF:BonusPermanent(value)
	self.permanent = self.permanent + value
	self:CalculateAttributes(0, 0)
end

function _2_DEF:BonusPts(caster, inflictor, stacks, percent, duration)
	local target = self:GetCaster()

	if IsServer() then
		target:AddNewModifier(caster, inflictor, "_2_DEF_modifier_stack", {
			duration = duration, stacks = stacks, percent = percent
		})
	end
end

function _2_DEF:CalculateAttributes(stacks, percent)
	...
	if IsServer() then
		local mod = self:GetCaster():FindModifierByName("_2_DEF_modifier")
		if mod then mod:SetStackCount(total) end
	end
end

function _2_DEF:EnableBasicUpgrade()
	self.basic_up = true
end

function _2_DEF:SetBounds(min, max)
	self.min = min
	self.max = max
	self:CalculateAttributes(0, 0)
end