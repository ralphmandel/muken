_2_DEX = class ({})
LinkLuaModifier( "_2_DEX_modifier", "modifiers/_2_DEX_modifier", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "_2_DEX_modifier_stack", "modifiers/_2_DEX_modifier_stack", LUA_MODIFIER_MOTION_NONE )

function _2_DEX:GetIntrinsicModifierName()
	return "_2_DEX_modifier"
end

function _2_DEX:GetBehavior()
...
function _2_DEX:OnOwnerSpawned()
	if self:GetCaster():IsIllusion() then return end
	local mods = self:GetCaster():FindAllModifiersByName("_2_DEX_modifier_stack")
...
function _2_DEX:OnHeroLevelUp()
	local caster = self:GetCaster()
	
	self:CheckLevelUp(false)
end

function _2_DEX:CheckLevelUp(trained)
	local caster = self:GetCaster()
...
function _2_DEX:BonusPermanent(value)
	self.permanent = self.permanent + value
	self:CalculateAttributes(0, 0)
end

function _2_DEX:BonusPts(caster, inflictor, stacks, percent, duration)
	local target = self:GetCaster()

	if IsServer() then
		target:AddNewModifier(caster, inflictor, "_2_DEX_modifier_stack", {
			duration = duration, stacks = stacks, percent = percent
		})
	end
end

function _2_DEX:CalculateAttributes(stacks, percent)
...
function _2_DEX:SetBounds(min, max)
	self.min = min
	self.max = max
	self:CalculateAttributes(0, 0)
end