_1_CON = class ({})
LinkLuaModifier( "_1_CON_modifier", "modifiers/_1_CON_modifier", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "_1_CON_modifier_stack", "modifiers/_1_CON_modifier_stack", LUA_MODIFIER_MOTION_NONE )

function _1_CON:GetIntrinsicModifierName()
	return "_1_CON_modifier"
end

function _1_CON:OnUpgrade()
	self:CalcBase()
end

function _1_CON:SetBasePts(base)
	self.base = base
	self:CalcBase()
end

function _1_CON:CalcBase()
	local caster = self:GetCaster()
	
	local mod = caster:FindModifierByName("_1_CON_modifier")
	if mod then mod:Base_CON(self.base) end
	self:CalculateAttributes(0, 0)
end

function _1_CON:AddFraction(value)
...
function _1_CON:OnOwnerSpawned()
	if self:GetCaster():IsIllusion() then return end
	local mods = self:GetCaster():FindAllModifiersByName("_1_CON_modifier_stack")
...
function _1_CON:BonusPermanent(value)
	self.permanent = self.permanent + value
	self:CalculateAttributes(0, 0)
end

function _1_CON:BonusPts(caster, inflictor, stacks, percent, duration)
	local target = self:GetCaster()

	if IsServer() then
		target:AddNewModifier(caster, inflictor, "_1_CON_modifier_stack", {
			duration = duration, stacks = stacks, percent = percent
		})
	end
end

function _1_CON:CalculateAttributes(stacks, percent)
